...
# Run a one-time command (e.g., run pipeline immediately with --now)
docker-compose run --rm cron npx tsx scripts/cron_wrapper.ts --now
```
...
└──────────┬──────────────────────────────────┘
           │
           ├─> orchestration (from cron_wrapper --now)
           │   ├─> khu_crawler.ts
...
# Test pipeline manually with --now
docker-compose run --rm cron npx tsx scripts/cron_wrapper.ts --now
```
